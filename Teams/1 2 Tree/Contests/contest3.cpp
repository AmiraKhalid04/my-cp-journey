...
void DFS(int root, vector<vector<int>> &adj_list, vector<bool> &vis, vector<bool> &marked, vector<ll> &count,
         vector<int> &parent)
{
    stack<int> s;
    s.push(root);
    ...
    while (s.empty() == 0)
    {
        ...
        if (!f)
        {
            count[s.top()] += total + marked[s.top()] ? 1ll : 0ll;
            count[parent[s.top()]] += count[s.top()];
...
void solve()
{
    ...
    vector<ll> primes = linearSieve(1e7 + 8);
    vector<bool> isPrime(1e7 + 7, false);
    int count = 0;

    for (int i = 0; i < n; i++)
    {

        if (find(primes.begin(), primes.end(), v[i]) != primes.end())
        {
            isPrime[v[i]] = true;
            count++;
        }
    ...
    for (int i = 0; i < n; i++)
    {
        if (isPrime[v[i]])
        {
            ...
            else if (primesCount > m)
            {
                primesCount = 1;
                ans.push_back(nonPrimesCount + 1);
                nonPrimesCount = 0;
            }
        }
        else if (slices)
