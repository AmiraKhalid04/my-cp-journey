    ...
    while (s.empty() == 0)
    {
        ...
        for (int i = 0; i < adj_list[p].size(); i++)
        {
            if (vis[adj_list[p][i]] == 0)
            {
                ...
                if (!isAdj[adj_list[p][i]])
                {
                    ans[root].push_back(adj_list[p][i]);
                    finalAns++;
                }
            }
        }
...
void solve()
{
    ...
    for (int i = 1; i <= n; i++)
    {
        ...
        vis.assign(n + 1, 0);
        isAdjacent.assign(n + 1, 0);

        for (int j = 0; j < adj_list[i].size(); j++)
        {
            isAdjacent[adj_list[i][j]] = 1;
        }

        DFS(i, adj_list, vis, isAdjacent);
    }
    cout << finalAns << endl;
    for (int i = 1; i <= n; i++)
    {
