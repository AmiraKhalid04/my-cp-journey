...
bool DFS(int root, vector<vector<int>> &adj_list, vector<int> &vis)
{
    stack<int> s;
    s.push(root);
    // cout << root << " ";
...
void topological(vector<vector<pair<int, ll>>> &adj, stack<int> &stk, int v)
{
...
void solve()
{
    ...
    while (q--)
    {
        ...
        {
            cin >> vec[i];
            maxElement = max(vec[i], maxElement);
        }

        if (k > 1 || vec[j - 1] == maxElement)
            cout << "yes";
        else
            cout << "no";
