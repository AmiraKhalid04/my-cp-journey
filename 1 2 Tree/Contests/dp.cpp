...
int getMinCostTab(int n, int maxNum, vector<int> &vec, map<int, pair<ll, vector<pair<int, ll>>>> &operations)
{
    vector<vector<ll>> dp(200 + 2, vector<ll>(200 + 2, 0));
    ...
    for (int i = 200; i >= 0; i--)
    {
        for (int prev = 200; prev >= 0; prev--)
        {
            ...
            if (operations[vec[i]].first)
            {
                ll nextCost = dp[i + 1][prevValue];

                if (nextCost != -1)
                {
                    minCost = min(minCost, operations[vec[i]].first + nextCost);
                    doable = true;
                }
            }

            for (pair<int, ll> op : operations[vec[i]].second)
            {
                int y = op.first;
                ll c = op.second;
                ...
                if (y >= prevValue)
                {
                    // int x = vec[i];
                    // vec[i] = y;
                    ll nextCost = dp[i + 1][y];

                    if (nextCost != -1)
                    {
                        minCost = min(minCost, c + nextCost);
                        doable = true;
                    }
                    // vec[i] = x;
                }
            ...
            if (prevValue == 0 || vec[i] >= prevValue)
            {
                ll nextCost = dp[i + 1][vec[i]];
...
void solve()
{
    ...
    map<int, pair<ll, vector<pair<int, ll>>>> operations;
    ...
    for (int i = 0; i < m; i++)
    {
        ...
        if (op == 1)
        {
            ...
            operations[x].second.push_back({y, c});
        }
        else
        {
            int x;
            ...
            if (operations[x].first == 0 || operations[x].first > c)
            {
                operations[x].first = c;
            }
        }
    }
    // vector<vector<ll>> dp(n, vector<ll>(maxNum + 1, -2));

    ll ans = getMinCostTab(n, maxNum, vec, operations);
