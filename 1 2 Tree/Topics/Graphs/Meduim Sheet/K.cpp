...
void DFS(int root, vector<vector<pair<int, int>>> &adj_list, vector<bool> &vis,
         ll minCost)
{
    stack<int> s;
    s.push(root);
    ...
    while (s.empty() == 0)
    {
        ...
        {
            if (vis[adj_list[p][i].first] == 0)
            {

                if (adj_list[p][i].first == 0)
                {
                }
                f = 1;
                vis[adj_list[p][i].first] = 1;
                s.push(adj_list[p][i].first);
                // cout << adj_list[p][i] << " ";
            }
        }
        if (!f)
...
void solve()
{
    ...
    vector<vector<pair<int, ll>>> adj_list(27);
    int notA = 0;
    ...
    while (m--)
    {
        ...
        if (c1 == 'a' && c2 != 'a' && freq[c1 - 'a'])
        {
            costA = min(costA, cost);
        }

        adj_list[c1 - 'a'].push_back({c2 - 'a', cost});
    }
    ...
    for (int i = 1; i < 27; i++)
    {
        if (freq[i])
        {
            topological(adj_list, nodes, 27, i);

            dist = shortestpath(adj_list, nodes, i);
        }
        // if (dist[0] != 1e16)
        ans = min(ans, dist[0]);
    }
